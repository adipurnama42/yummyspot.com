...
// CS & Admin: bisa lihat semua status (pending, rejected, approved)
// Owner: bisa lihat katalog miliknya sendiri apapun statusnya
// User biasa: hanya yang approved
$isStaff  = $visitor && in_array($visitor['role'], ['cs', 'admin']);
$ownerId  = $visitor['id'] ?? 0;

if ($isStaff) {
    $st = $db->prepare("SELECT c.*,cat.name AS cat_name,cat.icon AS cat_icon,u.fullname AS owner_name,u.username AS owner_username FROM catalogs c JOIN categories cat ON c.category_id=cat.id JOIN users u ON c.owner_id=u.id WHERE c.slug=?");
    $st->execute([$slug]);
} else {
    $st = $db->prepare("SELECT c.*,cat.name AS cat_name,cat.icon AS cat_icon,u.fullname AS owner_name,u.username AS owner_username FROM catalogs c JOIN categories cat ON c.category_id=cat.id JOIN users u ON c.owner_id=u.id WHERE c.slug=? AND (c.verification_status='approved' OR c.owner_id=?)");
...
if ($user) {
    ...
    $rs=$db->prepare("SELECT * FROM ratings WHERE user_id=? AND catalog_id=?"); $rs->execute([$user['id'],$cat['id']]); $myRating=$rs->fetch();
}
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['review'])) {
    requireLogin(); verifyCsrf();
    $rating = (int)($_POST['rating']??0); $review = trim($_POST['review']??'');
...
<main class="main" style="max-width:760px;margin:0 auto">
  ...
  <div class="alert <?= $bc ?>" style="margin-bottom:.85rem">
    ...
    <span style="margin-left:.5rem"><a href="<?= APP_URL ?>/cs/dashboard.php?tab=verify" style="font-weight:700;color:inherit;text-decoration:underline">Proses Verifikasi</a></span>
    <?php endif; ?></div>
  </div>
  <?php endif; ?>
  ...
  <div class="card" style="margin-bottom:.85rem">
    <div class="card-header">
      ...
      <?php if ($user && !$myRating): ?><button class="btn btn-primary btn-sm" onclick="openModal('review-modal')"><i class="fa-solid fa-pen"></i> Tulis Ulasan</button><?php endif; ?>
    </div>
    <div class="card-body">
      ...
      <?php endif; ?>
      <?php if (empty($reviews)): ?><div class="empty" style="padding:1.5rem"><div class="e-icon"><i class="fa-regular fa-star"></i></div><h3>Belum ada ulasan</h3></div>
      <?php else: ?>
  ...
  <div class="card">
    ...
    <div class="card-body" style="padding:.65rem">
      <div class="ig-grid">
        <?php foreach ($relPosts as $rp2): ?><a href="post.php?id=<?= $rp2['id'] ?>" class="ig-cell" style="border-radius:var(--r-sm);overflow:hidden">
      ...
      <form method="POST">
        ...
        <div style="text-align:center;margin-bottom:1rem">
          ...
          <div class="stars-inp" style="justify-content:center" id="star-inp">
            <?php for($i=1;$i<=5;$i++): ?><i class="fa-solid fa-star" data-val="<?= $i ?>"></i><?php endfor; ?>
          </div>
