...
$db = getDB();
$oid = $user['id'];
$cats   = $db->prepare("SELECT c.*,cat.name AS cat_name,(SELECT COUNT(*) FROM ratings WHERE catalog_id=c.id) AS rc FROM catalogs c JOIN categories cat ON c.category_id=cat.id WHERE c.owner_id=? ORDER BY c.created_at DESC");
$cats->execute([$oid]);
$mycats = $cats->fetchAll();
$sts    = $db->prepare("SELECT SUM(total_likes) AS lk,SUM(total_reviews) AS rv,AVG(avg_rating) AS ar FROM catalogs WHERE owner_id=? AND verification_status='approved'");
$sts->execute([$oid]);
$stats = $sts->fetch();
$revs   = $db->prepare("SELECT r.*,u.fullname,c.name AS cat_name FROM ratings r JOIN users u ON r.user_id=u.id JOIN catalogs c ON r.catalog_id=c.id WHERE c.owner_id=? ORDER BY r.created_at DESC LIMIT 5");
$revs->execute([$oid]);
$revs = $revs->fetchAll();
...
<div class="app-wrap">
  ...
  <main class="main">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.1rem">
      <div>
        <h1 style="font-family:'Nunito',sans-serif;font-size:1.2rem;font-weight:900">Dashboard</h1>
        <div class="text-dim text-sm">Selamat datang, <?= e($user['fullname']) ?></div>
      </div>
      <a href="catalog-create.php" class="btn btn-primary btn-sm"><i class="fa-solid fa-plus"></i> Tambah Katalog</a>
    </div>
    <div class="stats-grid">
      <div class="stat-card">
        <div class="stat-label"><i class="fa-solid fa-building-store" style="color:var(--accent)"></i> Katalog Aktif</div>
        <div class="stat-value" style="color:var(--accent)"><?= count(array_filter($mycats, fn($c) => $c['verification_status'] === 'approved')) ?></div>
        <div class="stat-sub">dari <?= count($mycats) ?> total</div>
      </div>
      <div class="stat-card">
        <div class="stat-label"><i class="fa-regular fa-heart" style="color:var(--red)"></i> Total Likes</div>
        <div class="stat-value" style="color:var(--red)"><?= fmtNum((int)($stats['lk'] ?? 0)) ?></div>
        <div class="stat-sub">semua katalog</div>
      </div>
      <div class="stat-card">
        <div class="stat-label"><i class="fa-regular fa-star" style="color:var(--amber)"></i> Total Ulasan</div>
        <div class="stat-value" style="color:var(--amber)"><?= fmtNum((int)($stats['rv'] ?? 0)) ?></div>
        <div class="stat-sub">rata-rata <?= number_format((float)($stats['ar'] ?? 0), 1) ?> ★</div>
      </div>
      <div class="stat-card">
        <div class="stat-label"><i class="fa-solid fa-clock-rotate-left" style="color:var(--blue)"></i> Pending</div>
        <div class="stat-value" style="color:var(--blue)"><?= count(array_filter($mycats, fn($c) => $c['verification_status'] === 'pending')) ?></div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:.85rem">
      <div class="card">
        ...
        <div style="padding:.4rem">
          <?php if (empty($mycats)): ?><div class="empty" style="padding:1.5rem">
              <div class="e-icon" style="font-size:1.5rem"><i class="fa-solid fa-store"></i></div>
              <h3 style="font-size:.85rem">Belum ada katalog</h3><a href="catalog-create.php" class="btn btn-primary btn-sm mt-2">Buat Sekarang</a>
            </div>
          <?php else: ?>
            <?php foreach ($mycats as $c): ?>
              <a href="catalog-edit.php?id=<?= $c['id'] ?>" style="display:flex;align-items:center;gap:.65rem;padding:.55rem .65rem;border-radius:var(--r-sm);transition:background .15s;text-decoration:none" onmouseover="this.style.background='var(--bg)'" onmouseout="this.style.background=''">
                ...
                <span class="badge <?= match ($c['verification_status']) {
      ...
      <div class="card">
        ...
        <div style="padding:.4rem">
          <?php if (empty($revs)): ?><div class="empty" style="padding:1.5rem">
              <div class="e-icon" style="font-size:1.5rem"><i class="fa-regular fa-star"></i></div>
              <h3 style="font-size:.85rem">Belum ada ulasan</h3>
            </div>
          <?php else: ?>
