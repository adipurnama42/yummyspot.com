...
$st = $db->prepare("
    SELECT c.*, cat.name AS cat_name, cat.icon AS cat_icon,
           u.fullname AS owner_name, u.username AS owner_username,
...
// ── Handle POST approve/reject SEBELUM header ────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'approve') {
        ...
        $db->prepare("INSERT INTO catalog_verifications (catalog_id, cs_id, status, note) VALUES (?,?,'approved',?)")
           ->execute([$id, $user['id'], $note ?: 'Disetujui oleh CS']);
        createNotif($cat['owner_id'], $user['id'], 'verified', $id,
            'Katalog "' . $cat['name'] . '" telah diverifikasi dan aktif!');
        ...
        $db->prepare("INSERT INTO catalog_verifications (catalog_id, cs_id, status, note) VALUES (?,?,'rejected',?)")
           ->execute([$id, $user['id'], $note]);
        createNotif($cat['owner_id'], $user['id'], 'rejected', $id,
            'Katalog "' . $cat['name'] . '" ditolak: ' . $note);
...
<aside class="sidebar dash-sidebar">
    ...
    <a href="dashboard.php?tab=catalogs" class="sb-item active"><i class="fa-solid fa-store si"></i> Semua Katalog</a>
    ...
    <div style="display:grid;grid-template-columns:1fr 300px;gap:1rem;">
        ...
        <div>
            ...
            <div class="card" style="margin-bottom:.85rem;">
                <div class="card-header"><span class="card-title"><i class="fa-solid fa-circle-info" style="color:var(--accent)"></i> Informasi Katalog</span></div>
                <div class="card-body">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:.65rem;">
                        <div><div style="font-size:.68rem;color:var(--text3);font-weight:700;margin-bottom:.2rem;">Nama</div><div style="font-weight:700;"><?= e($cat['name']) ?></div></div>
                        <div><div style="font-size:.68rem;color:var(--text3);font-weight:700;margin-bottom:.2rem;">Kategori</div><div><i class="fa-solid <?= e($cat['cat_icon']) ?> fa-xs" style="color:var(--accent)"></i> <?= e($cat['cat_name']) ?></div></div>
                        <div><div style="font-size:.68rem;color:var(--text3);font-weight:700;margin-bottom:.2rem;">Kota</div><div><i class="fa-solid fa-location-dot fa-xs" style="color:var(--red)"></i> <?= e($cat['city']) ?></div></div>
                        <div><div style="font-size:.68rem;color:var(--text3);font-weight:700;margin-bottom:.2rem;">Kontak</div><div><?= $cat['contact'] ? '<i class="fa-brands fa-whatsapp fa-xs" style="color:var(--green)"></i> '.e($cat['contact']) : '<span class="text-dim">—</span>' ?></div></div>
                        <div><div style="font-size:.68rem;color:var(--text3);font-weight:700;margin-bottom:.2rem;">Jam Buka</div><div><?= $cat['open_time'] ? substr($cat['open_time'],0,5).' – '.substr($cat['close_time'],0,5) : '<span class="text-dim">—</span>' ?></div></div>
                        <div><div style="font-size:.68rem;color:var(--text3);font-weight:700;margin-bottom:.2rem;">Terdaftar</div><div><?= date('d M Y', strtotime($cat['created_at'])) ?></div></div>
                    </div>
                    <?php if ($cat['address']): ?>
                    <div style="margin-top:.85rem;padding-top:.75rem;border-top:1px solid var(--border);"><div style="font-size:.68rem;color:var(--text3);font-weight:700;margin-bottom:.2rem;">Alamat</div><div style="font-size:.85rem;color:var(--text2);"><?= e($cat['address']) ?></div></div>
                    <?php endif; ?>
                    <?php if ($cat['description']): ?>
                    <div style="margin-top:.75rem;padding-top:.75rem;border-top:1px solid var(--border);"><div style="font-size:.68rem;color:var(--text3);font-weight:700;margin-bottom:.2rem;">Deskripsi</div><div style="font-size:.85rem;color:var(--text2);line-height:1.65;"><?= nl2br(e($cat['description'])) ?></div></div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="card" style="margin-bottom:.85rem;">
                ...
                <div class="card-body" style="padding:.65rem;">
                    <div class="ig-grid"><?php foreach ($gallery as $g): ?><div class="ig-cell" style="border-radius:var(--r-sm);overflow:hidden;"><img src="<?= e($g['image']) ?>" alt="" loading="lazy"></div><?php endforeach; ?></div>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="card">
                ...
                <div class="card-body" style="padding:.5rem;">
                    ...
                    <div style="display:flex;align-items:flex-start;gap:.6rem;padding:.6rem .5rem;border-bottom:1px solid var(--border);">
                        <div class="avatar av-36" style="background:<?= $rp[1] ?>;color:<?= $rp[0] ?>;flex-shrink:0;"><?= initials($rv['fullname']) ?></div>
                        <div style="flex:1;">
                            ...
                            <?php if ($rv['review']): ?><div style="font-size:.82rem;color:var(--text2);"><?= e(mb_strimwidth($rv['review'],0,120,'...')) ?></div><?php endif; ?>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="card" style="margin-bottom:.85rem;">
                <div class="card-body">
                    ...
                    <div style="display:flex;align-items:center;gap:.65rem;margin-bottom:.75rem;">
                        <div class="avatar av-44" style="background:<?= $op[1] ?>;color:<?= $op[0] ?>;"><?= initials($cat['owner_name']) ?></div>
                        <div><div style="font-weight:700;font-size:.88rem;"><?= e($cat['owner_name']) ?></div><div style="font-size:.73rem;color:var(--text3);">@<?= e($cat['owner_username']) ?></div></div>
                    </div>
                    <div style="font-size:.8rem;color:var(--text2);margin-bottom:.85rem;"><i class="fa-regular fa-envelope fa-xs" style="color:var(--blue);"></i> <?= e($cat['owner_email']) ?></div>
                    <a href="<?= APP_URL ?>/profile.php?u=<?= e($cat['owner_username']) ?>" target="_blank" class="btn btn-outline btn-sm w-100" style="justify-content:center;">
                        <i class="fa-solid fa-arrow-up-right-from-square fa-xs"></i> Lihat Profil
                    </a>
                </div>
            </div>
            ...
            <div class="card" style="margin-bottom:.85rem;">
                <div class="card-body">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem;">
                        <div style="text-align:center;background:var(--bg);border-radius:var(--r-sm);padding:.65rem;"><div style="font-family:'Nunito',sans-serif;font-size:1.2rem;font-weight:900;color:var(--amber);"><?= number_format($cat['avg_rating'],1) ?></div><div style="font-size:.65rem;color:var(--text3);">Rating</div></div>
                        <div style="text-align:center;background:var(--bg);border-radius:var(--r-sm);padding:.65rem;"><div style="font-family:'Nunito',sans-serif;font-size:1.2rem;font-weight:900;color:var(--blue);"><?= $cat['total_reviews'] ?></div><div style="font-size:.65rem;color:var(--text3);">Ulasan</div></div>
                        <div style="text-align:center;background:var(--bg);border-radius:var(--r-sm);padding:.65rem;"><div style="font-family:'Nunito',sans-serif;font-size:1.2rem;font-weight:900;color:var(--red);"><?= $cat['total_likes'] ?></div><div style="font-size:.65rem;color:var(--text3);">Likes</div></div>
                        <div style="text-align:center;background:var(--bg);border-radius:var(--r-sm);padding:.65rem;"><div style="font-family:'Nunito',sans-serif;font-size:1.2rem;font-weight:900;color:var(--accent);"><?= $wishCount ?></div><div style="font-size:.65rem;color:var(--text3);">Wishlist</div></div>
                        <div style="text-align:center;background:var(--bg);border-radius:var(--r-sm);padding:.65rem;grid-column:1/-1;"><div style="font-family:'Nunito',sans-serif;font-size:1.2rem;font-weight:900;color:var(--purple);"><?= $postCount ?></div><div style="font-size:.65rem;color:var(--text3);">Postingan</div></div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div style="display:flex;flex-direction:column;gap:.5rem;">
                        <?php foreach ($history as $h): $hColor=$h['status']==='approved'?'var(--green)':'var(--red)'; $hIcon=$h['status']==='approved'?'fa-check-circle':'fa-times-circle'; ?>
                        <div style="border-left:3px solid <?= $hColor ?>;padding:.5rem .65rem;background:var(--bg);border-radius:0 var(--r-sm) var(--r-sm) 0;">
                            ...
                            <?php if ($h['note']): ?><div style="font-size:.78rem;color:var(--text2);margin-top:.2rem;font-style:italic;">"<?= e($h['note']) ?>"</div><?php endif; ?>
                        </div>
                        <?php endforeach; ?>
                    </div>
                </div>
...
<div class="modal-ov" id="approve-modal">
    <div class="modal">
        ...
        <div class="modal-body">
            <p style="font-size:.875rem;color:var(--text2);margin-bottom:1rem;">Katalog <strong><?= e($cat['name']) ?></strong> akan diaktifkan dan tampil di halaman publik.</p>
            <form method="POST">
                <?= csrfField() ?>
                ...
                <div class="modal-foot">
                    ...
                    <button type="submit" class="btn btn-success btn-sm"><i class="fa-solid fa-check-circle fa-xs"></i> Ya, Setujui</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal-ov" id="reject-modal">
    <div class="modal">
        ...
        <div class="modal-body">
            <p style="font-size:.875rem;color:var(--text2);margin-bottom:1rem;">Katalog <strong><?= e($cat['name']) ?></strong> akan ditolak. Pemilik akan mendapat notifikasi.</p>
            <form method="POST">
                <?= csrfField() ?>
                <input type="hidden" name="action" value="reject">
                <div class="form-group">
                    <label>Alasan Penolakan <span style="color:var(--red)">*</span></label>
                    <textarea name="note" class="form-control" rows="3" placeholder="Jelaskan alasan penolakan..." required></textarea>
                </div>
                <div class="modal-foot">
                    <button type="button" class="btn btn-outline btn-sm" onclick="closeModal('reject-modal')">Batal</button>
                    <button type="submit" class="btn btn-danger btn-sm"><i class="fa-solid fa-times-circle fa-xs"></i> Ya, Tolak</button>
                </div>
            </form>
        </div>
    </div>
