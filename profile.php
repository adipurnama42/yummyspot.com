...
$fcSt = $db->prepare("SELECT COUNT(*) FROM follows WHERE following_id = ?");
$fcSt->execute([$profile['id']]);
$followers = (int)$fcSt->fetchColumn();
$fgSt = $db->prepare("SELECT COUNT(*) FROM follows WHERE follower_id = ?");
$fgSt->execute([$profile['id']]);
$following = (int)$fgSt->fetchColumn();
$pcSt = $db->prepare("SELECT COUNT(*) FROM posts WHERE user_id = ? AND status = 'published'");
$pcSt->execute([$profile['id']]);
$postCount = (int)$pcSt->fetchColumn();
$rcSt = $db->prepare("SELECT COUNT(*) FROM ratings WHERE user_id = ?");
$rcSt->execute([$profile['id']]);
$reviewCount = (int)$rcSt->fetchColumn();
        ...
        <?php if ($isCS && !$isOwn): ?>
          <div style="margin-top:1rem;padding-top:.85rem;border-top:1px solid var(--border);">
            ...
            <?php if ($profile['role'] === 'owner'):
              $ownerCats = $db->prepare("SELECT COUNT(*) FROM catalogs WHERE owner_id=?");
              $ownerCats->execute([$profile['id']]);
              $catTotal = (int)$ownerCats->fetchColumn();
              $approvedCats = $db->prepare("SELECT COUNT(*) FROM catalogs WHERE owner_id=? AND verification_status='approved'");
              $approvedCats->execute([$profile['id']]);
              $catApproved = (int)$approvedCats->fetchColumn();
            ?>
