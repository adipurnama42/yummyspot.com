...
// Mark all as read — SEBELUM output HTML
...
<main class="main" style="max-width:640px; margin:0 auto;">
    ...
    <div style="background:#fff;border:1px solid var(--border);border-radius:var(--r-lg);padding:4rem 2rem;text-align:center;">
        <div style="font-size:3rem;color:var(--border2);margin-bottom:1rem;"><i class="fa-regular fa-bell"></i></div>
        <h3 style="font-family:'Nunito',sans-serif;font-size:1rem;font-weight:800;margin-bottom:.4rem;color:var(--text2);">Belum ada notifikasi</h3>
        <p style="font-size:.85rem;color:var(--text3);">Notifikasi akan muncul saat ada aktivitas baru.</p>
    </div>
    ...
    <div style="background:#fff;border:1px solid var(--border);border-radius:var(--r-lg);overflow:hidden;">
        <?php foreach ($group as $i => $n):
            ...
            [$icon, $color, $desc] = $tm;
            $hasFrom = !empty($n['sender_name']);
            $sp = $hasFrom ? $pals[(($n['from_user_id'] ?? 1) - 1) % count($pals)] : ['#e5e7eb','#f9fafb'];
        ?>
        <div style="display:flex;align-items:flex-start;gap:.85rem;padding:.9rem 1rem;<?= !$isLast ? 'border-bottom:1px solid var(--border);' : '' ?>background:<?= !$n['is_read'] ? '#fffbf8' : '#fff' ?>;">
            ...
            <div style="flex:1;min-width:0;">
                <div style="font-size:.875rem;line-height:1.5;color:var(--text);">
                    <?php if ($hasFrom): ?>
                    <a href="<?= APP_URL ?>/profile.php?u=<?= e($n['sender_username']) ?>" style="font-weight:700;color:var(--text);">
                        <?= e($n['sender_name']) ?>
                    </a>
                    <?php endif; ?>
                    <?= e($n['message']) ?>
            ...
            <?php
            $link = null;
            if (in_array($n['type'], ['like','comment'])) $link = APP_URL . '/post.php?id=' . $n['target_id'];
            elseif ($n['type'] === 'follow')   $link = APP_URL . '/profile.php?u=' . ($n['sender_username'] ?? '');
            elseif (in_array($n['type'], ['verified','rejected'])) $link = APP_URL . '/owner/catalogs.php';
            ?>
            <?php if ($link): ?>
            <a href="<?= e($link) ?>" style="flex-shrink:0;width:32px;height:32px;border-radius:50%;background:var(--bg);display:flex;align-items:center;justify-content:center;color:var(--text3);transition:all .18s;"
               onmouseover="this.style.background='var(--accent)';this.style.color='#fff'"
               onmouseout="this.style.background='var(--bg)';this.style.color='var(--text3)'"
               title="Lihat">
